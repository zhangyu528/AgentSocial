...
*   **流程**:
    ...
    2.  **环境探测**: 自动扫描系统中的 Agent CLI（Gemini, Claude, Codex），识别可用状态。
    3.  **AI 核心选择**: 展示探测结果，引导用户选择已安装的 Agent（目前仅支持 Gemini CLI，其他显示 Coming Soon）。
    4.  **凭证输入**: 输入飞书 App ID、App Secret 及项目路径。
    5.  **诊断与配置**: 引导完成飞书后台配置并进行 4 项核心能力自检。
*   **输出**: 更新全局配置文件 `~/.agentsocial/settings.json`。
...
*   **路径加固**: 使用 **MD5 摘要算法** 对 `ChatID` 进行脱敏和路径平滑处理，防止目录遍历。
*   **存储路径**: `~/.agentsocial/sessions/{app_id}/{md5_chat_id}/`。
*   **权限同步**: 使用 **符号链接 (Symlink)** 或受限权限副本同步全局认证状态。
*   **目的**: 确保会话互不干扰，保护用户凭证不被扩散。
...
#### FR-11: 基于应用可见范围的访问控制
*   **授权检测**: 机器人启动时自动加载“应用可见范围”成员。
*   **Fail-Closed 策略**: 若调用权限接口失败，系统必须默认拒绝访问并记录安全错误日志。
*   **越权反馈**: 针对非授权成员，机器人返回访问受限提示，确保只有白名单内用户可下令。
