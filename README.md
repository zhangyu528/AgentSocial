...
- **🧠 深度隔离与权限：多会话与可见范围管理**
  - 为每个群聊、每个用户自动创建独立的 `.gemini` 运行环境。
  - **访问控制**: 自动同步飞书“应用可见范围”，只有授权成员才能控制 Agent，防止非相关人员误操作。
- **📢 状态感知：智能广播通知**
...
鑻ユ満鍣ㄤ汉鏃犳硶鎺ユ敹娑堟伅鎴栧彂閫佸崱鐗囷紝璇锋鏌ラ涔﹀紑鍙戣€呭悗鍙帮細

1.  **鍚敤鏈哄櫒浜**: 搴旂敤鍔熻兘 -> 鏈哄櫒浜 -> 鍚敤
2.  **鏍稿績鏉冮檺 (Scopes)**:
...
3.  **浜嬩欢璁㈤槄**:
    - 娣诲姞 `鎺ユ敹娑堟伅 (im.message.receive_v1)`。
4.  **鍗＄墖鍥炶皟**:
